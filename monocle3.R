Monocle3Simple <- function(
    ...
    assay = NULL,
    slot  = "counts",
    reduction = Seurat::DefaultReduction(srt),  # 复用的降维名（如 "umap"）
    reuse_umap = TRUE,      # 是否复用 Seurat 的 UMAP
    num_dim = 50,           # 自算时的 PCA 维数
    use_partition = TRUE,   # learn_graph 是否按分区
    root_cells = NULL,      # 不给则自动从最大簇取一个细胞作为根
    seed = 11,
    plot = FALSE            # 是否画一个伪时间图
){
  set.seed(seed)
